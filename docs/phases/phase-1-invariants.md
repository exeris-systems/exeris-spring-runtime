...
- **Guards:** `ExerisWireLevelRuntimeIntegrationTest` (6/6 wire-level scenarios green, including
  the in-flight drain scenario re-enabled at the kernel 0.11.0 pin — see the enforcement table at the
  end of this file and [`phase-1-milestone-status.md`](phase-1-milestone-status.md)),
...
|:---|:---|
| Exeris-owned ingress end-to-end | `ExerisWireLevelRuntimeIntegrationTest` (bind, body, 404, status, telemetry scope, in-flight drain at shutdown — 6/6 from the kernel 0.11.0 pin; see `phase-1-milestone-status.md`) |
