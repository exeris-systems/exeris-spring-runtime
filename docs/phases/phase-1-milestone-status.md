...
| request enters via Exeris | DONE | `ExerisHttpDispatcher.java` ; `ExerisWireLevelRuntimeIntegrationTest` (6/6) | Wire-level HTTP client round-trip through Exeris ingress proven (bind, body, 404, status, telemetry scope). |
...
| graceful shutdown (in-flight drain) | DONE | `ExerisWireLevelRuntimeIntegrationTest#pureMode_shutdownDrainsInFlightRequest_beforeIngressBecomesUnavailable` | Closed at the kernel 0.11.0 pin. Ingress reliably stops answering after shutdown (asserted by the bind test), **and** a request in flight when shutdown begins now completes and is answered: the kernel drain runs before transport teardown, with a distinct `draining` state keeping the write path alive. Blocked from 2026-08-02 to the pin bump against 0.10.2, where the drain ran last and the request was dropped without a response. |
...
- **1b** wire-level E2E ingress proof — `ExerisWireLevelRuntimeIntegrationTest` 6/6 green (the in-flight drain scenario re-enabled at the kernel 0.11.0 pin), kernel testkit fixture consumed.
...
- [x] Wire-level end-to-end runtime test demonstrates Exeris-owned HTTP ingress path with real bind + HTTP client round-trip (no servlet/reactive runtime ownership). — `ExerisWireLevelRuntimeIntegrationTest` 6/6 green
...
- [x] Graceful shutdown behavior is verified with in-flight request drain in wire-level runtime tests. — `pureMode_shutdownDrainsInFlightRequest_beforeIngressBecomesUnavailable`, re-enabled at the kernel 0.11.0 pin and confirmed by repeated runs rather than a single pass (it failed 7 of 12 runs against 0.10.2). Ingress-becomes-unavailable remains separately covered by `pureMode_bindsPort_routesRequest_and_cleansUpAfterFixtureAndContextClose`.
...
Current closure verdict: **M1 closed (2026-05-09); the exit-gate item re-opened on 2026-08-02 is
re-closed at the kernel 0.11.0 pin.** Closure was originally declared on the evidence available at
the time; the drain test passed then, was subsequently found to be flaky, and was then proven broken
against the pinned kernel — so the checkbox was re-opened rather than back-dated. It is now closed on
its own evidence: the kernel reordered its shutdown and the test is active and stable again. The
re-opened period stands in the record deliberately; it is what made the 0.7.0 hold legible.
