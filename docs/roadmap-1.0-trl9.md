...
| Phase 1 | Closed (2026-05-09) | Pure Mode ingress is proven end-to-end on the wire (`ExerisWireLevelRuntimeIntegrationTest` 5/6: bind, body, 404, status, telemetry scope). The sixth case — in-flight drain at shutdown — is **disabled against kernel 0.10.2**: the kernel drain (`PaqsScheduler.close()`, 60 s deadline) is sequenced after transport teardown, so an in-flight request is dropped without a response. Fixed upstream in kernel 0.11.0; re-enabled when the pin moves. `PureModeClasspathGuardTest` ships uniformly in autoconfigure/web/tx/data/actuator. `ExerisDispatcherAllocationBaselineTest` enforces ≤ 1024 B/req mean for empty-body GET dispatch (observed ≈ 277 B/dispatch). Invariants captured in `docs/phases/phase-1-invariants.md`. |
...
| Phase 3B-α (request scope + structured concurrency) | Promoted to 1.0 preview (0.6.0-preview, per ADR-029) | Kernel-independent: `ScopedValue` (GA on JDK 25). Delivers `ExerisRequestScope` and tenant/correlation ID propagation across kernel calls. *(The `StructuredTaskScope` fan-out helpers originally scoped here were withdrawn 2026-08-08 — ADR-029 obligations 2 and 6; the reactor now compiles preview-free.)* patterns. Default-off via `exeris.runtime.context.scope.enabled`. |
