...
**Status basis:** repository state as of 2026-08-19. Feature trains: Phase 4B closure 2026-05-11 (PR #27);
ADR-021 amendment 2026-05-13; ADR-027/028/029/030 acceptance, 0.6.0/0.7.0/0.8.0/0.9.0 resequence,
Phase 3B-α closed (PR #32), Phase 4C closed (PRs #34/35/36/37/38) — all 2026-05-17.

Since then, one planned train landed early and everything else was hardening. **The Spring Boot 4 dual
matrix (ADR-028) shipped in 0.7.0 rather than the 0.8.0 train this document schedules it for** — 0.7.0
stayed untagged while it was held for the kernel drain fix, so work merged in the meantime ships in it.
The release-train table below is left as written, with the row annotated: a train label is a scheduling
estimate, and rewriting history to match delivery hides that the estimate moved.

PRs #39–#66 are otherwise hardening and downstream-driven fixes (ADR-041 compat security under
`web-application-type=none`, kernel provider scope re-binding for the compat security filter and flow
steps, three fail-open closures in the compat Spring Security surface, compat JDBC coverage ratchet,
Hibernate bootstrap settings, CI build + per-module JaCoCo gates, ADR-067 binary neutrality, the
`@CompatibilityMode` marker relocation, the kernel bump to **0.11.0**, and the JDK 25 LTS baseline).  
...
| Phase 1 | Closed (2026-05-09) | Pure Mode ingress is proven end-to-end on the wire (`ExerisWireLevelRuntimeIntegrationTest` **6/6**: bind, body, 404, status, telemetry scope, and in-flight drain at shutdown). The drain case was disabled against kernel 0.10.2 — that kernel sequenced `PaqsScheduler.close()` after transport teardown, so an in-flight request was dropped without a response — and re-enabled at the 0.11.0 pin, on repeated runs rather than a single pass. `PureModeClasspathGuardTest` ships uniformly in autoconfigure/web/tx/data/actuator. `ExerisDispatcherAllocationBaselineTest` enforces ≤ 1024 B/req mean for empty-body GET dispatch (observed ≈ 277 B/dispatch). Invariants captured in `docs/phases/phase-1-invariants.md`. |
...
| Phase 3B-α (request scope + structured concurrency) | Promoted to 1.0 preview (0.6.0-preview, per ADR-029) | Kernel-independent: `ScopedValue` (GA on JDK 25). Delivers `ExerisRequestScope` and tenant/correlation ID propagation across kernel calls. *(The `StructuredTaskScope` fan-out helpers originally scoped here were withdrawn 2026-08-08 — ADR-029 obligations 2 and 6; the reactor now compiles preview-free.)* Default-off via `exeris.runtime.context.scope.enabled`. |
...
| Spring Boot 4 nominal compatibility | **Landed in 0.7.0** (ADR-028 scheduled 0.8.0-preview) | Dual-matrix CI claim that the 1.0 baseline supports **both** Spring Boot 3.5+ and Spring Boot 4.x. Not a feature toggle; a cross-cutting matrix invariant. Both lines build and test on every push, and an ADR-067 `binary-neutrality` job compares every Spring call-site descriptor across the two lines — added after two defects compiled cleanly on both while binding to different `HttpHeaders` methods. Spring Security 7 remains a separate axis (obligation 6), so this is nominal compatibility, not a support commitment. |
...
- **Conditional story:** Phase 3B is split between kernel-independent and kernel-gated halves. **3B-α (request scope + structured concurrency)** lands at 0.6.0-preview as a kernel-independent module (`ScopedValue<RequestScope>`, tenant/correlation ID propagation; ADR-029 — the `StructuredTaskScope` helpers were withdrawn 2026-08-08); default-off. **3B-β (W3C `traceparent` context propagation)** targets the 0.9.0-preview Spring-side train, gated on the kernel `TraceContext` carrier via `ScopedValue` — which the kernel defers to the consolidated 1.0 GA roadmap Sprint 0.12 (~v0.12), **not** `exeris-kernel` 0.8.0 (ADR-031). **3B-γ (OTel span/metric emission)** is gated on kernel `PrometheusOtlpTelemetrySink` — currently in the v0.8/v0.9 telemetry gap section without sprint commitment; if kernel slips to v0.9, 3B-γ slips to 1.0.x without affecting the 3B-α / 3B-β GA story.
...
| 0.6.0-preview | Phase 3B-α — Request Scope + Structured Concurrency (per ADR-029) | `ExerisRequestScope` (`ScopedValue`-backed), tenant/correlation ID propagation; opt-in via `exeris.runtime.context.scope.enabled`. *(The `StructuredTaskScope` fan-out helpers listed here originally were withdrawn 2026-08-08 with ADR-029 obligations 2 and 6 — `StructuredTaskScope` propagates the binding without them.)* | scope leak tests under concurrent execution, no `ThreadLocal` on hot paths, default-off bootstrap | TRL-6 to TRL-7 |
...
- Verify scope leak tests under concurrent execution; unbound-path and explicit-rebind coverage on `ExerisRequestScope`; no `ThreadLocal` on hot paths (existing `WallIntegrityTest` ban extends here). *(Tenant isolation across `StructuredTaskScope` forks was withdrawn as a criterion 2026-08-08: it verified the JDK, not this runtime.)*
...
> **This train's headline shipped early, in 0.7.0.** The bullets below are kept as the record of what
> was planned and are accurate as a description of what landed; only the version is different. What
> remains genuinely open for 0.8.0 is ADR-063 (`ExerisHttpSecurity` per-path route authorization,
> accepted and unblocked by the kernel 0.11.0 pin) and the ADR-068 `-preview` artefact line, whose
> preview-clean-kernel prerequisite is now also met.
