...
| Phase 3B-β (W3C `traceparent` context propagation) | Kernel-gated for 1.0 preview (0.9.0-preview target, per ADR-031 — reserved row in `exeris-docs/adr-index.md`; content lands in the follow-up PR for the 0.9.0-preview train) | Waits on `exeris-kernel` 0.8.0 Sprint 0.12 (W3C `traceparent` + `TraceContext` via `ScopedValue`). Bridges kernel `TraceContext` to Spring beans via `ScopedValue<TraceContext>`; HTTP client egress propagates `traceparent` header. |
...
- Co-deliver Phase 3B-β (W3C `traceparent` context propagation, ADR-031 first half) — gated on `exeris-kernel` 0.8.0 Sprint 0.12 having shipped the kernel `TraceContext` carrier via `ScopedValue`. If kernel 0.8.0 has not landed by 0.9.0-preview cut, 3B-β slips to 0.9.5-preview or 1.0.x; Phase 5 ships independently of 3B-β.
...
- **3B-β** (W3C `traceparent` propagation) is **kernel-gated on 0.8.0**. Lands at 0.9.0-preview (co-delivered with Phase 5). ADR-031 first half.
