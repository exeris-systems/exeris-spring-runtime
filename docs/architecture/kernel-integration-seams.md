...
the separate `CARRIER_INDEX` slot), so re-binding it is safe. The event/flow/graph *bridge
classes* avoid the same gap by reading captured references through their `*EngineSupplier`
rather than the slot.

**Flow-step re-binding (`KernelProviderScope`):** the gap re-opens inside application
`FlowStepAction` bodies, which execute on kernel flow scheduler worker virtual threads and — per
the `FlowStepAction` SPI contract — obtain cross-cutting providers through the `KernelProviders`
slots (the compat `ExerisDataSource` backing a repository called from a saga step reads
`PERSISTENCE_ENGINE` directly). `ExerisFlowDefinitionRegistrar` therefore decorates the kernel
`FlowDefinitionBuilder` with `ProviderScopedFlowDefinitionBuilder`, wrapping every registered
step action and compensation in `eu.exeris.spring.boot.autoconfigure.KernelProviderScope` — the
value-returning sibling of `KernelProviderBinder`, colocated with `ExerisRuntimeLifecycle`
because that is where the captured references live (and because the flow module's boundary guard
deliberately bans direct `eu.exeris.kernel.spi.persistence..` imports). Same posture as the
request path: unbound-only re-bind, `ScopedValue` only, mode-neutral.
...
Spring beans declaring saga shapes
    ← ExerisFlowDefinitionRegistrar (SmartLifecycle, phase MAX_VALUE - 99)
        → engine.plans().newDefinition(name)
        → ProviderScopedFlowDefinitionBuilder(builder, KernelProviderScope)   // wraps step actions
        → bean.define(scopedBuilder) → engine.plans().compile(def)
...
JDBC packages directly (architecture guard enforces this). Those worker threads do not
inherit the bootstrap `ScopedValue` scope, so the registrar wraps every step action and
compensation in `KernelProviderScope` (see Seam 5, flow-step re-binding) — without it, any
step body whose collaborators read `KernelProviders.PERSISTENCE_ENGINE` (the compat
`ExerisDataSource` path) fails with "PersistenceEngine is not bound in the current scope"
and the saga compensates straight from step 0.
